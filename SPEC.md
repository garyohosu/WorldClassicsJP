...
上限は**原文基準の文字数**で管理する。運用開始時は少量から始め、問題がなければ徐々に増やす。
...
- `state.json.translate_retry_count` を 1 ずつ増やしながら最大3回リトライする

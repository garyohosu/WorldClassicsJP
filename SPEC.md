...
- `port`
- `model`
- `daily_max_chars`（現在の1日の翻訳量上限文字数）
- `current_phase`（現在の運用フェーズ: `1` / `2` / `3`。§15 参照）
...
1つの作品が `complete` になった場合、`state.json` の進行状態を基準に次作品へ移行する。

次作品の選択条件：

- `works_master.json` はメタデータ専用とし、作品状態（`complete` / `failed` / `paused` 等）は持たせない
- v1（単作品直列運用）では `current_work_id` より後ろの `work_id` を昇順に走査し、`pd_verified = true` の最初の作品を次作品とする
- 複数作品キュー運用を行う場合は `queue_state.json` 等の別状態ファイルを用意し、実行可否をそこで判定する
...
1. **キュー枯渇**: v1（単作品直列）では `state.json` の `current_work_id` より後ろに `pd_verified = true` の作品が存在しない。複数キュー運用時は `queue_state.json` 等で判定する
...
- 進行状態を進めない（`current_part` / `next_work_id` / 成功側の進行状態は更新しない）
- エラーをログに記録する
...
- 本番パスへ反映する直前に、`git rev-parse HEAD` で取得した 40 桁 SHA を `pre_publish_head` として `state.json` の `pre_publish_head` フィールドに保存する
- `state.json.current_stage` を `publish` に設定する
- `state.json.publish_retry_count` を 1 ずつ増やし、次の実行サイクルで同一パートを最大3回リトライする
- **`publish_retry_count` が 3 に達した場合は `current_work_status` を `failed` に設定し、手動対応が必要な状態として処理を停止する**
- 公開成功時に `publish_retry_count` を 0 に戻し、`pre_publish_head` を `""` にリセットし、その後にのみ `current_part` / `next_work_id` を進める
- 公開失敗時は `/tmp_build` を破棄し、本番ファイルを変更しない
- `git commit` または `git push` が失敗した場合は、本番パスに反映した差分とローカル履歴を `state.json` の `pre_publish_head` SHA に復元し、`/tmp_build` を破棄し、`state.json` を進めない。復元完了後、`pre_publish_head` を `""` にリセットする
...
- `publish_retry_count`: 公開リトライ回数（3回で `failed` 確定）
- `pre_publish_head`: publish 開始直前に `git rev-parse HEAD` で記録した 40 桁 SHA。publish 成功時・ロールバック完了後は `""` にリセット
...
| complete | 個別作品の全パート公開完了 | Publisher が全パート完了を検出した時点。直後に LOAD_NEXT を実行し、次作品が見つかれば `active` へ遷移する（§16.1 参照） |
...
検出方法：`works_master.json` の SHA-256 ハッシュ値を `/data/works_master.hash` ファイルに保存する。補助ジョブ起動時に現在のハッシュと保存済みハッシュを比較し、差異がある場合のみジョブを実行する。処理完了後にハッシュを更新する。手動でも `works_master.hash` を削除することで強制再実行できる。
...
更新タイミング：Publisher ステップの完了後（必須 HTML 成果物の `/tmp_build` への生成後）、本番パスへの仮反映前に `/tmp_build` 内に生成する。仮反映時に HTML 成果物と一緒に本番パスへコピーされ、その後 GitHub にコミット・プッシュされる。
...
| 分類 | 条件 | 対応 |
|------|------|------|
| 補助成果物失敗 | XML 生成・書き込みが失敗するが、本文 HTML は正常生成済み | 警告ログを記録し、コミット処理を継続する |
| 致命的エラー | ファイルシステム障害・権限エラー等で補助成果物の書き込み自体が不能 | `publish failure` として扱い、`current_stage = publish` でリトライ待機する |

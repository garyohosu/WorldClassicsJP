...
- 同一実行内で解消しない場合は `current_segment_id` を維持したままその日の処理を終了し、翌日の実行で同一セグメントに再挑戦する
- `state.json.translate_retry_count` は同一実行内の再翻訳回数として扱う
- 翻訳に成功した時点で `translate_retry_count` を 0 に戻す
...
- `translate_retry_count`: 翻訳リトライ回数
...
- `preprocess` / `translate` / `quality_check` で中断した場合は、そのセグメントの翻訳処理を先頭からやり直す
...
画像取得は API を使わず、AI エージェントがブラウザ操作または直接 URL 取得で保存する。人手の手動操作は前提としない。
